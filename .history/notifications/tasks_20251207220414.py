...
def send_stock_alert_email(product_id, content_type_id):
    ...
    try:
        ...
        if model_class == Supplement:
            ...
            logger.warning(
                "Unknown product type for content_type_id: %s",
                content_type_id
            )
            return
...
Diet Planner System""".strip()
        ...
        logger.info(
            "Stock alert email sent successfully for product: %s to %d admin(s)",
            product.name,
            len(admin_emails)
        )
        ...
        logger.error("Product or content type not found: %s", e)
    except (ConnectionError, TimeoutError, OSError) as e:
        logger.error("Network error sending stock alert email: %s", e)
    except (ValueError, TypeError, AttributeError) as e:
        # Catch specific unexpected errors that might occur
        logger.error(
            "Unexpected error sending stock alert email: %s",
            e,
            exc_info=True
        )
...
def send_stock_alert_email_async(product_id, content_type_id):
    ...
    thread.start()
    logger.debug(
        "Started background thread for sending stock alert email "
        "(product_id: %s)",
        product_id
    )
