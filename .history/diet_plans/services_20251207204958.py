...
Calculates BMR, TDEE, and generates personalized meal plans
...
class DietPlanGenerator:
    """
    Generate personalized diet plans based on user profile.

    Uses BMR (Basal Metabolic Rate) and TDEE (Total Daily Energy Expenditure)
    calculations to create customized meal plans.
    """
    def __init__(self, user_profile):
        """
        Initialize diet plan generator with user profile.

        Args:
            user_profile: Profile instance containing user's physical attributes
        """
    ...
    def calculate_bmr(self):
        ...
        Returns:
            float: BMR in calories per day, or None if required data missing
        """
        required_fields = [
            self.profile.age,
            self.profile.height,
            self.profile.current_weight,
            self.profile.gender
        ]
        ...
        if self.profile.gender == 'male':
            bmr = (
                88.362 +
                (13.397 * self.profile.current_weight) +
                (4.799 * self.profile.height) -
                (5.677 * self.profile.age)
            )
        else:
            bmr = (
                447.593 +
                (9.247 * self.profile.current_weight) +
                (3.098 * self.profile.height) -
                (4.330 * self.profile.age)
            )
    ...
    def adjust_calories_for_goal(self, tdee, goal_type):
        ...
        Args:
            ...
            goal_type: 'weight_loss' or 'weight_gain'
    ...
    def generate_meal_plan(self, daily_calories, goal_type):
        """
        Generate meal plan with food suggestions based on goal.
    ...
    def generate(self):
        ...
        Calculates BMR, TDEE, adjusts for goal, and creates meal plan.
        Saves or updates DietPlan in database.

        Returns:
            DietPlan: Generated diet plan instance, or None if generation fails
        """
        ...
        daily_calories = self.adjust_calories_for_goal(
            tdee,
            self.profile.fitness_goal
        )
        meals = self.generate_meal_plan(
            daily_calories,
            self.profile.fitness_goal
        )
